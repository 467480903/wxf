...
def chassis_move_forward(robot, distance=2.0):
    """底盘向前移动"""
    print(f"\n=== 向前移动 {distance} 米 ===")
    try:
        from chassis_controller import ChassisController
        with ChassisController() as ctrl:
            state = ctrl.move_forward(distance)
            print(f"✅ 向前移动完成, state={state}")
    except Exception as e:
        print(f"❌ 向前移动失败: {e}")


def chassis_move_backward(robot, distance=2.0):
    """底盘向后移动"""
    print(f"\n=== 向后移动 {distance} 米 ===")
    try:
        from chassis_controller import ChassisController
...
def chassis_rotate_right(robot, angle=90):
    """底盘向右转（顺时针）"""
    print(f"\n=== 向右转 {angle} 度 ===")
    try:
        from chassis_controller import ChassisController
        with ChassisController() as ctrl:
            state = ctrl.rotate(-angle)  # 负值 = 顺时针
            print(f"✅ 向右转完成, state={state}")
    except Exception as e:
        print(f"❌ 向右转失败: {e}")


def chassis_rotate_left(robot, angle=90):
    """底盘向左转（逆时针）"""
    print(f"\n=== 向左转 {angle} 度 ===")
    try:
        from chassis_controller import ChassisController
        with ChassisController() as ctrl:
            state = ctrl.rotate(angle)  # 正值 = 逆时针
            print(f"✅ 向左转完成, state={state}")
    except Exception as e:
        print(f"❌ 向左转失败: {e}")
...
def main():
    ...
    try:
        # 1. move_ee_pose_open_2.py
        move_ee_pose_open_2(robot)
        time.sleep(1)
        
        # 2. move_arm_by_json_grab_above_1.py
        move_arm_by_json_grab_above_1(robot)
        time.sleep(2)
        
        # 3. move_waist_by_json_down.py
        move_waist_by_json_down(robot)
        time.sleep(2)
        
        # 4. 向前5m
        chassis_move_forward(robot, 2.0)
        time.sleep(8)
        
        # 5. move_ee_pose_close_2.py
        move_ee_pose_close_2(robot)
        time.sleep(1)
        
        # 6. offset_move_pull.py
        offset_move_pull(robot)
        time.sleep(1)
        
        # 7. 向后退5m
        chassis_move_backward(robot, 2.0)
        time.sleep(8)
        
        # 8. 向右转90度
        chassis_rotate_right(robot, 90)
        time.sleep(1)
        ...
        # 16. 向左转90度
        chassis_rotate_left(robot, 90)
        time.sleep(1)
        
        # 17. move_arm_by_json_default.py - 手臂复位
