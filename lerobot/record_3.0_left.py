...
MQTT_PORT = 1883
MQTT_TOPIC = "/record_lerobot_dataset_left"
MQTT_LOG_TOPIC = "/record_lerobot_dataset_left/log"
...
CAMERAS: List[CameraSpec] = [
    ...
    CameraSpec(
        ...
        video_name="observation.images.hand_left_color"
    ),
]
...
def record_thread_function(args):
    ...
    last_frames = {}
    joint_names = [
        "idx51_arm_l_joint1", "idx52_arm_l_joint2", "idx53_arm_l_joint3",
        "idx54_arm_l_joint4", "idx55_arm_l_joint5", "idx56_arm_l_joint6", "idx57_arm_l_joint7",
        "left_gripper"
    ]

    log_message(f"开始录制 Episode {args.episode_id} (左臂)...")
    ...
    try:
        while is_recording:
            ...
            if js and "states" in js:
                ...
                # 获取左臂夹爪张开角度
                if end_state and "left_end_state" in end_state:
                    left_end = end_state["left_end_state"]
                    if "end_states" in left_end and left_end["end_states"]:
                        # 假设夹爪关节是第一个关节
                        gripper_position = left_end["end_states"][0]["position"]
                    else:
...
def on_message(client, userdata, msg):
    ...
    try:
        ...
        if cmd == "start":
            ...
            out_dir = Path(payload.get("output_dir", "lerobot_dataset_left"))
