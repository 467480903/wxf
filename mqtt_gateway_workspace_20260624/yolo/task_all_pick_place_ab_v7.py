#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""V7 low-risk continuity-focused continuous A/B pick-place runner.

V7 keeps V6's action order and safety boundaries, then only improves PNC busy recovery.
When a previous navigation task is still running, V7 waits for PNC to become idle before retrying
instead of repeatedly submitting doomed goto requests.
...
If V7 is not suitable onsite, run V6, V5, V4, or V1 directly:
...
def _load_child_sequence(script_path: Path) -> list[str]:
    namespace: dict[str, Any] = runpy.run_path(str(script_path), run_name=f"_wxf_v7_{script_path.stem}")
...
def _apply_v7_profile() -> dict[str, str]:
    """Apply V7 low-risk continuity profile and return values for log visibility."""
    profile = {
        "G2_WXF_TTS_PRE_PLAY_DELAY_S": os.environ.get("G2_WXF_V7_TTS_PRE_PLAY_DELAY_S", "0.0"),
        "G2_WXF_NAV_IDLE_STABLE_S": os.environ.get("G2_WXF_V7_NAV_IDLE_STABLE_S", "0.15"),
        "G2_WXF_NAV_IDLE_WAIT_POLL_S": os.environ.get("G2_WXF_V7_NAV_IDLE_WAIT_POLL_S", "0.15"),
        "G2_WXF_NAV_BUSY_WAIT_IDLE_BEFORE_RETRY": os.environ.get("G2_WXF_V7_NAV_BUSY_WAIT_IDLE_BEFORE_RETRY", "1"),
        "G2_WXF_NAV_SKIP_PRE_IDLE_FIRST": os.environ.get("G2_WXF_V7_NAV_SKIP_PRE_IDLE_FIRST", "1"),
        "G2_WXF_NAV_CANCEL_EXISTING_FIRST": os.environ.get("G2_WXF_V7_NAV_CANCEL_EXISTING_FIRST", "1"),
        "G2_WXF_NAV_IDLE_WAIT_TIMEOUT_S": os.environ.get("G2_WXF_V7_NAV_IDLE_WAIT_TIMEOUT_S", "120.0"),
        "G2_WXF_NAV_BUSY_RETRY_DELAY_S": os.environ.get("G2_WXF_V7_NAV_BUSY_RETRY_DELAY_S", "0.10"),
        "G2_WXF_NAV_POLL_INTERVAL_S": os.environ.get("G2_WXF_V7_NAV_POLL_INTERVAL_S", "0.15"),
        "G2_WXF_FAST_WHOLE_BODY_SPLIT_DELAY_S": os.environ.get("G2_WXF_V7_FAST_WHOLE_BODY_SPLIT_DELAY_S", "0.02"),
        "G2_WXF_YOLO_RESIDENT": os.environ.get("G2_WXF_V7_YOLO_RESIDENT", "1"),
        "G2_WXF_MP3_INLINE": os.environ.get("G2_WXF_V7_MP3_INLINE", "1"),
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="V7 low-risk MQTT runner for pick A, place A, pick B, and place B"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
    ...
    profile = _apply_v7_profile()
    print("# yolo/task_all_pick_place_ab_v7.py")
    print(f"# children={len(CHILDREN)}, mode={'execute' if args.execute else 'dry-run plan'}")
    print("# baseline_v6=yolo/task_all_pick_place_ab_v6.py")
    print("# optimization=V6 profile + wait-idle-before-busy-retry navigation recovery")
    for key, value in profile.items():
        print(f"# v7_profile: {key}={value}")
    ...
    print(f"# v7_total_timing: status=done duration_s={total_duration_s:.3f}", flush=True)
    return 0
