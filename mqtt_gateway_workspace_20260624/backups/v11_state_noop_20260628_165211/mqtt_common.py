...
def _run_fast_sequence_python(base: Path, target: Path, args: list[str]) -> bool:
    ...
    if name == "offset_move_vertical_b.py":
        ...
        forward_bias_m = 0.015
        biased_depth_offset = raw_depth_offset + forward_bias_m
        depth_offset = max(-0.02, min(0.085, biased_depth_offset))
        print(f"vertical_b raw_depth_offset={raw_depth_offset:.6f}m, forward_bias_m={forward_bias_m:.6f}m, biased_depth_offset={biased_depth_offset:.6f}m, clamped_depth_offset={depth_offset:.6f}m")
        run_ee_offsets(source_script, (depth_offset, 0.0, 0.0), (depth_offset, 0.0, 0.0))
        return True
