...
def _run_fast_sequence_python(base: Path, target: Path, args: list[str]) -> bool:
    ...
    if name == "move_ee_pose_close_2.py":
        # Mirror Robot/move_ee_pose_close_2.py exactly: right tool closes first,
        # then left tool after the original 50 ms delay.
        run_gripper("close", source_script=source_script, targets={"right": 0.0})
        time.sleep(0.05)
        run_gripper("close", source_script=source_script, targets={"left": 0.0})
        return True
